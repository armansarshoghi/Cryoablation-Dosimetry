...
def posterior_predictive(model, idata, rows, likelihood):
    """Posterior predictive draws, expressed as death percentages.

    The count models predict n_live out of n_expected; converting to a
    percentage puts all three observation models on one scale so their interval
    coverage can be compared directly.
    """
    import pymc as pm
    with model:
        pp = pm.sample_posterior_predictive(idata, random_seed=7, progressbar=False)
    obs = pp.posterior_predictive['obs'].stack(s=('chain', 'draw')).values
    if likelihood == 'gaussian':
        return obs
    n_exp = rows['n_expected'].values[:, None]
    return 100.0 * (n_exp - obs) / n_exp


def interval_coverage(pred, observed, probs=(0.50, 0.95)) -> dict:
    """Fraction of observations inside each central posterior predictive interval."""
    out = {}
    for prob in probs:
        lo = np.percentile(pred, 100 * (1 - prob) / 2, axis=1)
        hi = np.percentile(pred, 100 * (1 + prob) / 2, axis=1)
        out[prob] = float(np.mean((observed >= lo) & (observed <= hi)))
    return out
